...
@Tag(name = "Owner — Email", description = "Gửi email và quản lý automation rule theo giải đấu — requires OWNER role")
...
public class OwnerEmailController {
	...
	@Operation(summary = "Danh sách mẫu email khả dụng (toàn hệ thống + riêng của tôi)")
	...
	public ResponseEntity<ApiResponse<PageResponse<EmailTemplateResponse>>> listTemplates(
		...
		return ResponseEntity.ok(ApiResponse.success(
				emailTemplateService.listTemplatesForOwner(extractUserId(authentication), page, size)));
	}
